...
The previous SoA must have run their code on some datasets, including both simulated and real.  Identify those settings.  We will be comparing our algorithm's performance on those settings as well, so as to not be cherry-picking settings.
...
Run your algorithm on the simulations written above as follows. For each simulation setting:
